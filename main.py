...
def main():
    ...
    # algorithm-------------------------------------------------
    if file_path == 'simsTxt/50nodes.txt':
    ...
    for community_id in range(num_communities):
        ...
        diffuse = Diffuse(dim, epsilon, sub_n_nodes,  beta_start, beta_end, orderSize,
                          epochs, batch_size, n_steps)
        diffuse.fit1(subX)

        for id_subject in range(subject_num):
            start = int(id_subject * (orderSize))
            end = int((id_subject + 1) * (orderSize))
...
if __name__ == "__main__":
    ...
    file_path = 'simsTxt/sim200.csv'
    gtrue_path = 'simsTxt/stand_200nodes.txt'
    main()